...
./deploy.sh remote <user>@<host>      # over ssh; assumes NOPASSWD sudo on the host
./deploy.sh musl remote <user>@<host> # static musl build for older systems
```

`deploy.sh` detects the target host's architecture and builds for it (native
`cargo`, or `cross` for a different arch or musl), then installs the binary,
D-Bus policy and systemd unit and enables the service. An optional first
argument picks the C library — `glibc` (default) or `musl` (a static binary).
...
client *read*; restrict it to a group if you want fewer readers.
